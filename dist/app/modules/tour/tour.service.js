...
const getSingleTour = async (slug) => {
    const tour = await tour_model_1.Tour.findOne({ slug });
    return {
        data: tour,
    };
};
const updateTour = async (id, payload) => {
    ...
    const updatedTour = await tour_model_1.Tour.findByIdAndUpdate(id, payload, { new: true });
    if (payload.deleteImages && payload.deleteImages.length > 0 && existingTour.images && existingTour.images.length > 0) {
...
const getSingleTourType = async (id) => {
    const tourType = await tour_model_1.TourType.findById(id);
    return {
        data: tourType
    };
};
const updateTourType = async (id, payload) => {
    ...
    const updatedTourType = await tour_model_1.TourType.findByIdAndUpdate(id, payload, { new: true });
